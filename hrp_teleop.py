...
class HRP_Teleop(object):
  ...
  def init(self):
    ...
    rospy.Subscriber('chatter', String, self.callback_otto_navi) #Added by Jojz
  ...
  def callback_otto_navi(self,data): #added by jojz
     self.ottoData = data.data
  ...
  def get_otto_navi(self): #added by jojz
    key = self.ottoData
    return key
    #hee hee


	#changed from (1) ch = self.get_key() and (2) ch = self.get_otto_navi. uncomment/comment for otto_navi/keys  
  ...
  def run(self):
    try:
      ...
      while not rospy.is_shutdown():
        #ch = self.get_key() #changed from ch =  self.get_key() ch = to self.get_otto_navi uncomment dio
	ch = self.get_otto_navi()
        self.process_key(ch)
        self.update()
        self.showstatuslines()
        r.sleep()
